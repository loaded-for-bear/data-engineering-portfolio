...
### UUID 生成: faker vs uuid 標準ライブラリ

UUID 生成では **標準ライブラリの `uuid` の方が速い**。faker は内部で uuid を呼んだ上にラッパー処理が入るため遅い。

```
uuid.uuid4()  x 100,000: 0.225s（50万件換算: 約1.1秒）
faker.uuid4() x 100,000: 0.300s（50万件換算: 約1.5秒）
```

UUID が必要なだけなら `uuid.uuid4()` を使う。faker は人名・住所など **faker でしか作れないデータ** に限定する。
→ 詳細: [uuid.md](./uuid.md)
